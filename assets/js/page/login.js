function Login() {
	...
	this.getTabHeader = function getTabHeader() {
		var tabHeader = "<li role=\"presentation\" class=\"active\"><a href=\"#article\" id=\"home-tab\" role=\"tab\" data-toggle=\"tab\" aria-controls=\"home\" aria-expanded=\"true\">"
				+ msg("login") + "</a></li>";
		if (isAuthorized) {
			tabHeader += "<li role=\"presentation\" class=\"\"><a href=\"#massupdate\" role=\"tab\" id=\"massupdate-tab\" data-toggle=\"tab\" aria-controls=\"profile\" aria-expanded=\"false\">"
				+ msg("massupdate") + "</a></li>";
	...
	this.getTabContent = function getTabContent(isAuthorized, value1, value2) {
		...
		if (isAuthorized) {
			tabContent += "<div role=\"tabpanel\" class=\"tab-pane fade\" id=\"massupdate\" aria-labelledby=\"profile-tab\"> ";
			tabContent += "<div id=\"massupdatecnt\">Loading files. Please wait</div> ";
			tabContent += "</div> ";
...
this.loadUploadTab = function loadUploadTab(id) {
	...
	for ( var o in oJson) {
		...
		var hr = "doGetUploadFile('#uploadedfilescnt'.'"+oJson[o].id+"')";
	...
	this.addEvents = function addEvents() {
		...
		$("#massupdate-tab").on("click",	function() {
			$.getScript('./assets/js/plugins/massupdate/massupdate.js', function() {				
				var s = new MassUpdateGUI().getGUI();
				$("#massupdatecnt").html(s);
